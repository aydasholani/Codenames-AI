...
def save_card_images(img, textbox_boxes, recognized_texts, image_name):
    ...
    for i, (box, text) in enumerate(zip(textbox_boxes, recognized_texts), start=1):
        x1, y1, x2, y2 = box
        text_img = img[y1:y2, x1:x2]
        
        # Save each image
        # output_card_path = os.path.join(output_dir, f"card_{i}.png")
        # cv2.imwrite(output_card_path, text_img)
